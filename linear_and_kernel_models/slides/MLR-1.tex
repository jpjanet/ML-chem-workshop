...
\begin{align*}
    &\hat{y}_{MLR}(X) = \begin{bmatrix}  1 &  x^{(1)}_{1} & x^{(1)}_{2} & \hdots x^{(1)}_{d}  \\  \vdots  &&&\vdots \\  1&  x^{(n)}_{1} & x^{(n)}_{2} & \hdots x^{(n)}_{d} \end{bmatrix}  \begin{bmatrix} w_0 \\ w_1 \\ \vdots \\ w_d  \end{bmatrix}   = Xw\\
\end{align*}
}
\vspace{-1.0cm}
